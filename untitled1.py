...
while demi == 1:
    ...
    plt.savefig('annual-salary-with-education.png')
    plt.show()
    plt.clf()

    #dot
    databyyear.drop(['Post-secondary'], axis=1, inplace=True)
    databyyear.drop(['overall'], axis=1, inplace=True)
    lag_plot(databyyear)
    plt.rcParams['figure.figsize'] = (16.0, 12.0)
    plt.savefig('salary-education-relation.png')
    plt.show()
...
while demi == 2:
    #bruh
    df = pd.read_excel("D5250018C2019QQ02C.xlsx")
